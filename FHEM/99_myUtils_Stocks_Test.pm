...
sub sendNotificationDefine_Test($$$){
...
sub sendNotification_Test($){
	...
	if ($last_price != $current_price) {
		...
		if ($rounded >= $change_relative) {
			...
			fhem("set telebot msg \@Angela Fehlende Referenzdaten zum angegebenen Zeitintervall\n 
					Kurs für $stock_name ist zum Startpreis € $first_price um $print gestiegen");
			}
			...
			fhem("set telebot msg \@Angela Fehlende Referenzdaten zum angegebenen Zeitintervall\n 
					Kurs für $stock_name ist zum Startpreis € $first_price um $print gefallen");
			}
			else {
...
sub contacts_Test(){
	...
	my $json_str = encode_json(\@matches);
	return $json_str;

}

sub addAtDevice_Test($$$$){
	my($deviceName, $hours, $percentage, $user) = @_;
	my $borderTime = time();
	my $name = "at_" . "$deviceName" . "_" . "$user" ."_" ."$hours" . "_" . "$percentage";
	
	fhem("define $name at +*02:00:00 {sendNotificationFunction(\"$deviceName\", \"$hours\", \"$percentage\", \"$borderTime\", \"$user\")}");
	fhem("attr $name room Aktien");
	
	return undef;
}

sub sendNotificationFunction_Test($$$$$){
	my ($deviceName, $hours, $percentage, $borderTime, $user) = @_;
	my ($currentTime);
	my ($targetTime);
	my ($before);
	my (@valString);
	...
	for(my $i = 0; $i < $length - 1; $i++){
		...
		if($lastVal <= $minChange){
			fhem("set telebot msg \@Angela Kurs für $deviceName ist in den letzten $hours h um $percentage gefallen");
			return "Nachricht gesendet";
		}
		
		if($lastVal >= $maxChange){
			fhem("set telebot msg \@Angela Kurs für $deviceName ist in den letzten $hours h um $percentage gestiegen");
			return "Nachricht gesendet";
		}
	}
	
	return "Nichts gesendet";
}

sub getAtDevices_Test(){
	my (@atDevices) = defInfo('TYPE=at:FILTER=room=Aktien', 'NAME'); 
	my $json_str = encode_json(\@atDevices);
	return $json_str;
}
